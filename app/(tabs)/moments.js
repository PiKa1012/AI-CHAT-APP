...
export default function MomentsScreen() {
  ...
  useEffect(() => {
    setFilterAuthor(params.filterAuthor || null);
  }, [params.filterAuthor]);
      ...
      if (newMomentId) {
        for (const ai of aiCharacters) {
          if (Math.random() > 0.3) {
            ...
            await sendLocalNotification('收到点赞', `${ai.name} 赞了你的朋友圈`, { type: 'like' });
          }
        }
        ...
        <TouchableOpacity style={styles.momentCard} activeOpacity={0.8} onPress={() => router.push({ pathname: '/moment-detail', params: { momentId: item.id } })}>
          <View style={styles.momentBody}>
            <TouchableOpacity onPress={() => isUser ? router.push('/profile') : router.push({ pathname: '/ai-profile', params: { id: item.author_id } })}>
              <SafeAvatar uri={isUser ? userProfile.avatar : getAuthorAvatarUri('ai', item.author_id)} size={42} name={getAuthorName(item.author_type, item.author_id)} color={getAvatarColor(item.author_type, item.author_id)} />
            </TouchableOpacity>
              ...
              {item.images?.length > 0 && (
                <View style={styles.momentImages}>
                  {item.images.slice(0, 9).map((img, i) => (
                    <Image key={i} source={{ uri: img }} style={item.images.length === 1 ? styles.momentImageSingle : styles.momentImage} resizeMode="cover" />
                  ))}
                </View>
              )}
  ...
  const renderHeader = () => {
    ...
    const coverSource = targetAI ? targetAI.coverBg : userProfile.coverBg;
    const onAvatarPress = targetAI ? () => router.push({ pathname: '/ai-profile', params: { id: targetAI.id } }) : () => router.push('/profile');

    return (
      <View style={styles.coverWrap}>
        <Animated.View style={[styles.coverBg, { opacity: !filterAuthor ? headerOpacity : 1, backgroundColor: targetAI ? avatarColor : '#3b82c4' }]}>
          {coverSource ? (
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      {filterAuthor && (
        <TouchableOpacity style={styles.backOverlay} onPress={() => { setFilterAuthor(null); router.replace({ pathname: '/(tabs)/moments' }); }} activeOpacity={0.6}>
      ...
      {selectedMoment && (() => {
        ...
        <View style={styles.commentBar}>
          ...
          {replyTarget && <View style={styles.commentBarReply}><Text style={styles.commentBarReplyText}>回复 @{getAuthorName(replyTarget.author_type, replyTarget.author_id)}：{replyTarget.content.slice(0, 20)}{replyTarget.content.length > 20 ? '...' : ''}</Text></View>}
        </View>
        );
      })()}
...
const styles = StyleSheet.create({
  ...
  coverBg: { position: 'absolute', top: 0, left: 0, width: SCREEN_WIDTH, height: COVER_HEIGHT, backgroundColor: '#3b82c4' },
  ...
  coverName: { color: '#fff', fontSize: 18, fontWeight: '600', marginBottom: 4 },
  coverBio: { color: 'rgba(255,255,255,0.75)', fontSize: 13, marginBottom: 12 },
  ...
  backOverlay: { position: 'absolute', top: 12, left: 12, zIndex: 200, width: 32, height: 32, borderRadius: 16, backgroundColor: 'rgba(0,0,0,0.3)', justifyContent: 'center', alignItems: 'center' },

  momentCard: { backgroundColor: '#fff', marginBottom: 8, position: 'relative' },
  momentBody: { flexDirection: 'row', padding: 14, gap: 10 },
  momentContent: { flex: 1 },
  momentAuthor: { fontSize: 15, fontWeight: '600', color: '#4A90D9', marginBottom: 4 },
  momentText: { fontSize: 15, color: '#333', lineHeight: 22 },
  momentImages: { flexDirection: 'row', flexWrap: 'wrap', gap: 4, marginTop: 8 },
  momentImage: { width: (SCREEN_WIDTH - 100) / 3, height: (SCREEN_WIDTH - 100) / 3, borderRadius: 4, backgroundColor: '#f0f0f0' },
  momentImageSingle: { width: SCREEN_WIDTH - 100, height: (SCREEN_WIDTH - 100) * 0.6, borderRadius: 4, backgroundColor: '#f0f0f0' },
  momentTime: { fontSize: 12, color: '#999' },
  momentFooter: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 },
  momentActions: { flexDirection: 'row', gap: 16 },
  actionBtn: { flexDirection: 'row', alignItems: 'center', gap: 3 },
  actionText: { fontSize: 12, color: '#999', marginLeft: 2 },

  likesBar: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f8f8f8', padding: 8, borderRadius: 4, marginTop: 6, gap: 4 },
  likesText: { fontSize: 13, color: '#666', flex: 1 },

  commentsBox: { backgroundColor: '#f8f8f8', borderRadius: 4, marginTop: 6, padding: 8 },
  ...
  imagePickerRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, paddingHorizontal: 16, paddingBottom: 24 },
  imageThumbWrap: { position: 'relative' },
  imageThumb: { width: (SCREEN_WIDTH - 64) / 3, height: (SCREEN_WIDTH - 64) / 3, borderRadius: 6, backgroundColor: '#f0f0f0' },
  imageRemoveBtn: { position: 'absolute', top: -8, right: -8, zIndex: 10, backgroundColor: '#fff', borderRadius: 10 },
  addImageBtn: { width: (SCREEN_WIDTH - 64) / 3, height: (SCREEN_WIDTH - 64) / 3, borderRadius: 6, borderWidth: 1, borderColor: '#ddd', borderStyle: 'dashed', justifyContent: 'center', alignItems: 'center', backgroundColor: '#fafafa' },

  commentBar: { borderTopWidth: 1, borderTopColor: '#eee', backgroundColor: '#fff' },
  commentBarRow: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 6, paddingVertical: 8, gap: 6 },
  commentBarReply: { paddingHorizontal: 14, paddingBottom: 6 },
  commentBarReplyText: { fontSize: 12, color: '#999' },
  commentInput: { flex: 1, borderWidth: 1, borderColor: '#ddd', borderRadius: 20, paddingHorizontal: 14, paddingVertical: 8, fontSize: 15 },
  commentSendBtn: { paddingHorizontal: 14, paddingVertical: 8 },
  commentSendText: { fontSize: 15, color: '#4A90D9', fontWeight: '500' },
});
