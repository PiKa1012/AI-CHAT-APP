...
const I = ({ label, value, onChangeText, placeholder, hint }) => (
  <View>
    <Text style={st.label}>{label}</Text>
    <TextInput style={st.input} value={value} onChangeText={onChangeText} placeholder={placeholder} placeholderTextColor="#999" />
  ...
  return (
    <ScrollView style={st.ctn}>
      <View style={st.sec}>
        ...
        {music && <View style={st.sub}><I label="API 地址" value={mu} onChangeText={setMu} placeholder="https://xxx.vercel.app" hint="部署 NeteaseCloudMusicApi Enhanced 后的地址" /><B onPress={() => doTest(testMusic, '音乐')} text="测试连接" disabled={!!testing} /></View>}
        <SW label="🗺️ 高德地图" desc="位置天气路线查询" value={map} onChange={setMap} />
        {map && <View style={st.sub}><I label="Web API Key" value={mk} onChangeText={setMk} placeholder="输入高德 Web 服务 API Key" hint='在控制台 → 应用管理 → 创建应用后获取 Key，启用「Web 服务」' /><B onPress={() => doTest(testMap, '地图')} text="测试连接" disabled={!!testing} /></View>}
      </View>
